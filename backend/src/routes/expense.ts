...
expenseRouter.post('/', isAuthenticated, async (c) => {
  try {
    ...
    if (!body.invoiceDate || !body.vendor || !body.description || !body.amount) {
...
expenseRouter.put('/:id', isAuthenticated, async (c) => {
  try {
    ...
    const existingRecord = await db.select().from(expense)
      .where(and(eq(expense.id, id), eq(expense.userId, userId)))
      .limit(1);
    ...
    // Update the record
    const updatedExpense = await db.update(expense)
      .set({
