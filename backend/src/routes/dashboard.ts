...
dashboardRouter.get("/summary/:year", isAuthenticated, async (c) => {
	try {
		...
		const totalIncome = Number.parseInt(totalIncomeResult[0].total) || 0;
		const totalExpenses = Number.parseInt(totalExpensesResult[0].total) || 0;
...
dashboardRouter.get("/monthly/:year", isAuthenticated, async (c) => {
	try {
		...
		const monthlyBreakdown = months.map((month) => {
			...
			const expensesForMonth = monthlyExpenses.find(
