...
dashboardRouter.get('/monthly/:year', isAuthenticated, async (c) => {
  try {
    ...
    const monthlyIncome = await db
      .select({
        month: sql`EXTRACT(MONTH FROM ${income.invoiceDate})::integer`,
      ...
      .where(
        and(
          eq(income.userId, userId),
          sql`EXTRACT(YEAR FROM ${income.invoiceDate}) = ${year}`
        )
      )
      .groupBy(sql`EXTRACT(MONTH FROM ${income.invoiceDate})`)
      .orderBy(sql`EXTRACT(MONTH FROM ${income.invoiceDate})`);
    
    // Get monthly expenses
    const monthlyExpenses = await db
      .select({
        month: sql`EXTRACT(MONTH FROM ${expense.invoiceDate})::integer`,
      ...
      .where(
        and(
          eq(expense.userId, userId),
          sql`EXTRACT(YEAR FROM ${expense.invoiceDate}) = ${year}`
        )
      )
      .groupBy(sql`EXTRACT(MONTH FROM ${expense.invoiceDate})`)
      .orderBy(sql`EXTRACT(MONTH FROM ${expense.invoiceDate})`);
